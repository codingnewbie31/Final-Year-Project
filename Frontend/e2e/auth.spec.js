...
test.describe("Signup Page", () => {
  ...
  test("should load signup page with all fields", async ({ page }) => {
    ...
    await expect(
      page.locator('input[placeholder="Create a strong password"]'),
    ).toBeVisible();
  ...
  test("should highlight selected role", async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Job Seeker")')).toHaveClass(
      /border-blue-500/,
    );

    await page.click("text=Employer");
    await expect(page.locator('button:has-text("Employer")')).toHaveClass(
      /border-blue-500/,
    );
  });
  ...
  test("should toggle password visibility", async ({ page }) => {
    ...
    await page.click(
      'button[type="button"]:near(input[placeholder="Create a strong password"])',
    );
    await expect(passwordInput).toHaveAttribute("type", "text");
  });
